...
/**
 * Generate a human-readable name from session key.
 */
function generateSessionName(sessionKey: string): string {
  if (sessionKey === "agent:main:main") {
    return "Main Session"
  }

  // Chat sessions: agent:main:trap:{slug}:{chatId}
  const chatMatch = sessionKey.match(/^agent:main:trap:([^:]+):(.+)$/)
  if (chatMatch) {
    return `Chat: ${chatMatch[1]}`
  }

  // Agent sessions: agent:main:cron:*:trap-{taskId}
  const agentMatch = sessionKey.match(/:trap-([a-f0-9-]+)$/)
  if (agentMatch) {
    return `Agent: ${agentMatch[1].slice(0, 8)}`
  }

  // Reviewer sessions: agent:main:cron:*:trap-pr-review-{taskId}
  const reviewerMatch = sessionKey.match(/:trap-pr-review-([a-f0-9-]+)$/)
  if (reviewerMatch) {
    return `Reviewer: ${reviewerMatch[1].slice(0, 8)}`
  }

  // Cron sessions
  if (sessionKey.startsWith("agent:main:cron:")) {
    return `Cron: ${sessionKey.split(":").pop() ?? "unknown"}`
  }
...
type SessionStatus = "active" | "completed" | "stale"
  ...
  private async flush(): Promise<void> {
    ...
    for (const sessionKey of sessionsToSync) {
      ...
      sessionInputs.push({
        ...
        type,
        status,
        model: info?.lastAssistantMessage?.model ?? "unknown",
        project_slug: projectSlug,
        task_id: taskId,
        tokens_input: info?.lastAssistantMessage?.usage.input,
        tokens_output: info?.lastAssistantMessage?.usage.output,
        tokens_cache_read: info?.lastAssistantMessage?.usage.cacheRead,
        tokens_cache_write: info?.lastAssistantMessage?.usage.cacheWrite,
        tokens_total: info?.lastAssistantMessage?.usage.total,
        // Cost is in the usage object but sessionFileReader doesn't extract it yet
        // We'll need to enhance it or parse from raw message
        cost_input: undefined,
        cost_output: undefined,
        cost_cache_read: undefined,
        cost_cache_write: undefined,
        cost_total: undefined,
        file_path: watchedSession.filePath,
        file_mtime_ms: info?.fileMtimeMs,
        created_at: now, // Will be ignored on update
        updated_at: now,
        completed_at: info?.isDone ? info.fileMtimeMs : undefined,
        last_activity_at: info?.fileMtimeMs,
        stop_reason: info?.lastAssistantMessage?.stopReason,
        is_terminal_error: info?.isTerminalError,
        output_preview: info?.lastAssistantMessage?.textPreview,
      })
    }
