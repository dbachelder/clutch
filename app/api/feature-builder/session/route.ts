...
export async function POST(request: NextRequest) {
  try {
    ...
    const sessionId = await (convex as unknown as { mutation: (name: string, args: unknown) => Promise<string> }).mutation(
      "featureBuilder:createSession",
      {
        project_id,
        user_id,
      }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await (convex as unknown as { mutation: (name: string, args: unknown) => Promise<unknown> }).mutation(
      "featureBuilder:updateSessionProgress",
      {
        id,
        current_step,
        completed_steps,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const mutate = (convex as unknown as { mutation: (name: string, args: unknown) => Promise<unknown> }).mutation
